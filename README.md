...
**Per-step movement clip** (default 1.25× median — strongly recommended on) — a compressor for the blocks. The last trained block always hogs learning (it sits closest to the output, so its gradient is the most coherent) and over-edits fine detail — distorted eyes are the classic symptom. The clip caps how far any block may move *within a single step* at N× the median block's step, shortening the overshoot before it lands. Only that one step is affected, so nothing the block already learned is touched. Unlike turning blocks off or slowing a range, it targets whoever *actually* runs hot, so it keeps working whatever Blocks to Train is set to. Turn it off only for a deliberate A/B — every quality problem it prevents compounds from epoch 1.
...
The learning rate is yours to set — there's no automatic throttle on top of it. Use warmup and the per-step clip below, and judge the rate from your samples.
