...
**Previews default to 768×768, 56-frame clips with sound** — a short watchable clip with the model's generated audio, muxed into a playable video the gallery opens with a click (never autoplay). That's the regime H3 was trained in, and how you hear a voice LoRA converge without leaving Fizgig; without the audio VAE set, clips simply render silent. The **Sample length** dropdown has stills (seconds per preview) and other lengths when you want them. Set the **Turbo LoRA** in Preferences and previews render in **6 steps instead of 20**, with the Turbo applied at 75% on top of your training LoRA for the render only — previews only, never the saved LoRA; steps and strength are adjustable on the Samples tab. A preview that outgrows VRAM steps itself down a ladder rather than dying — a shorter clip first (56 → 22 frames), then resolution (768×640, 640×640, on to a 512×512 floor) — triggered by a hard OOM *or* by detecting the Windows paging crawl — with a note that below 768 the model is outside its training canvas, and the size that fit saved as the new default so the next run starts there.
