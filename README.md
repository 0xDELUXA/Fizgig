...
**How it works:** pick **MiniMax H3 (experimental)** from the Base Model selector at the top of the Training tab, and the usual flow applies — Start-tab folder, Captions, Samples, Training. The base trains frozen and quantized with your LoRA in bf16 on top, so a **32 GB card trains comfortably with everything on defaults**. Leave **Blocks Swap on Auto**: the trainer plans block swap and gradient checkpointing from your **free** VRAM at launch — not your card's size, so close ComfyUI first. Around 30 GB free runs with no block swap at all, up to 1 MP. Below that the base is what's in the way at ~21 GB, and the *same file* can load 4-bit instead — ~11 GB resident for a little more error in the frozen model, which puts 0.25 MP within reach of ~22 GB free with no swap. That's a command-line option today (`--base_quant nf4`); there's no switch for it on the Training tab yet. Mileage varies with your dataset and bucket sizes, and if you hit an out-of-memory you can set **Blocks Swap** to a number instead of Auto to override the planner outright. **Adaptive LR**, **LoKR**, **Pause/Resume** and **in-training previews** are all wired.
...
| Preset | Settings |
|---|---|
| **✨ MiniMax H3 Defaults** | LoKR factor 8, dim/alpha 16, 60 epochs, lr 1e-4 with Adaptive LR off, 0.25 MP, 60% low noise + mid-concentrated, `adamw` |
