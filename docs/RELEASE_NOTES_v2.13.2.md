...
Groundwork so a phone-facing run monitor can ship later as an ordinary app update rather than a
new image: the pod's password is now available to Fizgig itself (a generated one previously never
reached the app), and port 8081 is reserved for it.
