...
> ### 📰 Latest news
> - **Fizgig 3.0 trains LoKR — and it's now the Krea 2 default.** LoKR (LyCORIS Kronecker) covers the whole weight matrix with structure instead of a thin low-rank slice — in our validation runs it produced the **highest likeness we have ever measured** with this app's own scorer, with noticeably more natural skin and light than standard LoRA on the same dataset and settings. One **Factor** dial replaces rank and alpha, output loads straight into ComfyUI, and Repair Studio / LoRA the Explorer edit and save LoKR **natively — lossless, no conversion**. Pick any Krea 2 preset and it's already on; standard LoRA stays one click away. [Training ↓](#training)
...
- **LoKR training (Krea 2, the default)** — the LyCORIS Kronecker parametrization the community rates highest for likeness, now trainable with the loss watch, adaptive LR and auto-recaption attached. One **Factor** dial replaces rank/alpha (lower factor ≈ more capacity, bigger file: factor 8 ≈ 400 MB, 16 ≈ 100 MB). Output is standard LyCORIS format — drops straight into ComfyUI — and Repair Studio / Explorer edit and re-save it natively, lossless. Prefer standard LoRA? It's the other entry in the Network Type dropdown.
