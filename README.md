...
**Previews render as short clips.** H3 is a video model, so samples come out as 56-frame clips at 640×640 that you can scrub in the gallery — the regime the model was actually built for. Both are dropdowns on the Samples tab: raise the length or the size when you want a preview to be judged rather than glanced at, and remember a clip costs minutes rather than seconds, so set **Generate every N epochs** to match.
...
**Adapter-relative LR** (**on by default at `0.003`** — also `0.005` and `0.01` fast build) — the answer to the oldest problem in LoRA training: the rate that's safe at the start is far too slow later, and the rate that's right later wrecks a fresh adapter. A LoRA begins at zero, so the very first steps are enormous *relative to its own size*, which is where epoch-1 distortion comes from. With it on, the Learning Rate box is a **ceiling** rather than a setting: the run starts well below it and works its way up, holding each step at a fixed fraction of the adapter's current size. You get a gentle start without picking a warmup length, and full speed later without picking the moment. The shipped preset uses the slow build; raise the fraction if you want it to climb faster, or set it Off for a flat run at the box value. Either way the console prints the adapter size, the growth rate and the percentage of the ceiling in use at every epoch.
