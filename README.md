...
**How it works:** pick **MiniMax H3 (experimental)** from the Base Model selector at the top of the Training tab, and the usual flow applies — Start-tab folder, Captions, Samples, Training. The base trains frozen and quantized with your LoRA in bf16 on top, so a **32 GB card trains comfortably with everything on defaults**. On smaller cards, leave **Blocks Swap on Auto**: the trainer plans block swap and gradient checkpointing from your **free** VRAM at launch — not your card's size, so close ComfyUI first. Roughly: ~30 GB free runs the int8 base with no block swap up to 1 MP; ~22 GB runs the NF4 base with no swap at 0.25 MP; ~14 GB uses swap. Mileage varies with your dataset and bucket sizes, and if you hit an out-of-memory you can set **Blocks Swap** to a number instead of Auto to override the planner outright. **Adaptive LR**, **LoKR**, **Pause/Resume** and **in-training previews** are all wired.
