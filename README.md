...
**Adapter-relative LR** (default Off — `0.003` slow build, `0.005`, `0.01` fast build) — the answer to the oldest problem in LoRA training: the rate that's safe at the start is far too slow later, and the rate that's right later wrecks a fresh adapter. A LoRA begins at zero, so the very first steps are enormous *relative to its own size*, which is where epoch-1 distortion comes from. Switch this on and the Learning Rate box becomes a **ceiling** rather than a setting: the run starts well below it and works its way up, holding each step at a fixed fraction of the adapter's current size. You get a gentle start without picking a warmup length, and full speed later without picking the moment. Pick the fraction by how fast you want it to build; the console prints the adapter size, the growth rate and the percentage of the ceiling in use at every epoch.
