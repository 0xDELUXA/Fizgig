...
**High-LR smoothing** (default Warmup 2 epochs, EMA 0.99) — what makes a high static LR trainable. **Warmup** ramps the LR up over the first epochs, so full-size steps never land on the untrained adapter — that's where the worst distortion comes from. **EMA** saves (and previews) a smoothed running average of the weights instead of the raw values: big steps zigzag around the good solution, and the average is the center of the zigzag. Training itself always runs on the raw weights, so neither costs speed. Warmup is ignored when Adaptive LR is on; EMA works with both.

**Movement governor** (default 0.22) — the speed limit that makes the Learning Rate box safe. Distortion tracks how far the weights *move*, not the LR number — and the same LR moves a LoKR ~10× further than a LoRA. The governor measures the movement every step and throttles the effective LR to hold the clean rate, so the LR you set is a **ceiling**, not a dose: set it high and the governor spends the excess as headroom. The budget is **per training step**, so it scales with your dataset automatically — a 250-image set gets proportionally more movement per epoch than a 40-image one, because an epoch on it contains proportionally more training. The console reports what your budget works out to per epoch at launch. Works with warmup and EMA.
